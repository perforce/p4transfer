...
== FetchTransfer vs Helix native DVCS functions

See also link:P4Transfer.html[P4Transfer] for reasons when that would be better than FetchTransfer.
...
IMPORTANT: When it is an option, using Helix native DVCS is preferred.  But if that can’t be used for some reason, FetchTransfer can pretty much always be made to work.
