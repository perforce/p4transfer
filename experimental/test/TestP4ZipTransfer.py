# -*- encoding: UTF8 -*-
# Tests for the P4ZipTransfer.py module.
...
import P4ZipTransfer       # noqa: E402
...
REMOTE_NAME = "transfer_remote"
TRANSFER_CONFIG = "transfer.yaml"

TEST_COUNTER_NAME = "P4ZipTransfer"
INTEG_ENGINE = 3
...
class P4Server:
    ...
    def p4cmd(self, *args):
        ...
        if not self.logger:
            self.logger = logutils.getLogger(P4ZipTransfer.LOGGER_NAME)
...
class TestP4ZipTransfer(unittest.TestCase):
    ...
    def __init__(self, methodName='runTest'):
        ...
        if test_logger is None:
            ...
            logutils.resetLogger(P4ZipTransfer.LOGGER_NAME)
        self.logger = test_logger
        super(TestP4ZipTransfer, self).__init__(methodName=methodName)

    def assertRegex(self, *args, **kwargs):
        if python3:
            return super(TestP4ZipTransfer, self).assertRegex(*args, **kwargs)
        else:
            return super(TestP4ZipTransfer, self).assertRegexpMatches(*args, **kwargs)
    ...
    def run_P4ZipTransfer(self, *args):
        base_args = ['-c', self.transfer_cfg, '-s']
        if args:
            base_args.extend(args)
        pt = P4ZipTransfer.P4ZipTransfer(*base_args)
        result = pt.replicate()
        return result
    ...
    def testArgParsing(self):
        ...
        pt = P4ZipTransfer.P4ZipTransfer(*args)
    ...
    def testConfigValidation(self):
        ...
        self.assertRegex(msg, "Option remote_name must not be blank")
    ...
    @unittest.skip("Wildcards can cause problems")
    ...
    @unittest.skip("zip/unzip can't handle obliterated delete")
    ...
    @unittest.skip("zip/unzip can't handle moves where source is obliterated")
    ...
    @unittest.skip("zip/unzip can't handle moves not fully included")
    def testMoves(self):
    ...
    @unittest.skip("Unzip can't find archive in zip file")
    ...
    def testRemoteIntegs(self):
        ...
        filelog = self.target.p4.run_filelog('//depot/import/inside_file1')
        self.assertEqual(filelog[0].revisions[0].action, 'import')  # Different to P4Transfer
    ...
    @unittest.skip("Zip file missing archive")
    ...
    def testIntegMultipleToOne(self):
        ...
        self.assertEqual(len(filelog.revisions[0].integrations), 3)
        self.assertEqual(filelog.revisions[0].integrations[0].how, "copy from")
        self.assertEqual(filelog.revisions[0].integrations[1].how, "merge from")  # Diff from P4Transfer
        self.assertEqual(filelog.revisions[0].integrations[2].how, "add from")    # Diff from P4Transfer
    ...
    def testIntegDeleteProblem(self):
        ...
        filelog = self.target.p4.run_filelog('//depot/import/inside_file1')
        self.assertEqual(filelog[0].revisions[0].action, 'branch')  # Diff from P4Transfer
    ...
    def testIntegDeleteIgnore(self):
        ...
        self.assertEqual(files[2]['depotFile'], '//depot/import/inside_file3')
        self.assertEqual(files[3]['depotFile'], '//depot/import/inside_file4')
    ...
    def testIntegIgnoreAndEdit(self):
        ...
        self.assertEqual(len(filelog.revisions[0].integrations), 1)
        self.assertEqual(filelog.revisions[0].integrations[0].how, "ignored")
        self.assertEqual(filelog.revisions[0].action, "integrate")   # Diff from P4Transfer
    ...
    def testKeywords(self):
        ...
        # self.run_P4ZipTransfer("--nokeywords")
        self.run_P4ZipTransfer()

        newFiles = self.target.p4cmd('files', "//...")
        self.assertEqual(len(newFiles), 4)
        self.assertTrue(newFiles[0]['type'] in ["text+k", "ktext"])
        self.assertTrue(newFiles[1]['type'] in ["kxtext", "text+kx"])
        self.assertEqual(newFiles[2]['type'], "text+kmx")
        self.assertEqual(newFiles[3]['type'], "text+kmx")
        ...
        self.assertTrue(files[0]['type'] in ["ktext", "text+k"])
