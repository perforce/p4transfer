# -*- encoding: UTF8 -*-
# Tests for the CopySnapshot.py module.
...
TEST_COUNTER_NAME = "CopySnapshot"
LOGGER_NAME = "TestCopySnapshot"
...
class TestCopySnapshot(unittest.TestCase):
    ...
    def __init__(self, methodName='runTest'):
        ...
        super(TestCopySnapshot, self).__init__(methodName=methodName)

    def assertRegex(self, *args, **kwargs):
        if python3:
            return super(TestCopySnapshot, self).assertRegex(*args, **kwargs)
        else:
            return super(TestCopySnapshot, self).assertRegexpMatches(*args, **kwargs)
    ...
    def run_CopySnapshot(self, *args):
        self.logger.debug("-----------------------------Starting CopySnapshot")
        base_args = ['-c', self.transfer_cfg]
        if args:
            base_args.extend(args)
        obj = CopySnapshot.CopySnapshot(*base_args)
        result = obj.run()
        return result
